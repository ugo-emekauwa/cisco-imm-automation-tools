<h1 align="center">Base UCS Chassis Profile Maker</h1>
...
<br>
<p align="center">
  The Base UCS Chassis Profile Maker for Cisco Intersight automates the creation of base UCS Chassis Profiles without Policies.
</p>
<br>

## Features
- Automatically build an Intersight base UCS Chassis Profile without Policies to your exact specification. Anything that can be done through the Intersight GUI can be done here and more!

- Assign the base UCS Chassis Profile to a UCS Chassis in Intersight, using the serial, name, model, or PID (product ID).

- Quickly restore UCS Chassis Profile configurations as part of your on-prem Intersight backup solution.
...
2. Within the unzipped Cisco IMM Automation Tools repository, navigate to the folder \src\profile_makers\ucs_chassis_profile_maker.
3. Edit the ucs_chassis_profile_maker.py file to set the **`key_id`** variable using the following instructions:

    **(a).** Open the ucs_chassis_profile_maker.py file in an IDLE or text editor of choice.
    ...
    **(d).** Fill in between the quotes of the **`key_id`** variable value with the ID of your API key. For example:
      ...
      ```
4. Edit the ucs_chassis_profile_maker.py file to set the **`key`** variable using the following instructions:
    ...
    **(d).** Fill in between the quotes of the **`key`** variable value with your system's file path to the SecretKey.txt file for your API key. For example:
      ...
      ```
5. Edit the ucs_chassis_profile_maker.py file to set all the configuration variable values using the following instructions:
...
6. Save the changes you have made to the ucs_chassis_profile_maker.py file.
7. Run the ucs_chassis_profile_maker.py file.

## Where to Demo
The Base UCS Chassis Profile Maker can be demoed on Cisco dCloud in the following content:
