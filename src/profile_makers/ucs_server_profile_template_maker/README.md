<h1 align="center">Base UCS Server Profile Template Maker</h1>

<br>
<p align="center">
  <img alt="Profile Template Makers Title Image" title="Profile Template Makers" src="./assets/Profile_Template_Makers_Title_Graphic.png">
</p>  
<br>
<p align="center">
  The Base UCS Server Profile Template Maker for Cisco Intersight automates the creation of base UCS Server Profile Templates without Policies.
</p>
<br>

## Features
- Automatically build an Intersight base UCS Server Profile Template without Policies to your exact specification. Anything that can be done through the Intersight GUI can be done here and more!

- Quickly restore UCS Server Profile Template configurations as part of your on-prem Intersight backup solution.
...
2. Within the unzipped Cisco IMM Automation Tools repository, navigate to the folder \src\profile_makers\ucs_server_profile_template_maker.
3. Edit the ucs_server_profile_template_maker.py file to set the **`key_id`** variable using the following instructions:

    **(a).** Open the ucs_server_profile_template_maker.py file in an IDLE or text editor of choice.
    ...
    **(d).** Fill in between the quotes of the **`key_id`** variable value with the ID of your API key. For example:
      ...
      ```
4. Edit the ucs_server_profile_template_maker.py file to set the **`key`** variable using the following instructions:
    ...
    **(d).** Fill in between the quotes of the **`key`** variable value with your system's file path to the SecretKey.txt file for your API key. For example:
      ...
      ```
5. Edit the ucs_server_profile_template_maker.py file to set all the configuration variable values using the following instructions:
...
6. Save the changes you have made to the ucs_server_profile_template_maker.py file.
7. Run the ucs_server_profile_template_maker.py file.

## Where to Demo
The Base UCS Server Profile Template Maker can be demoed on Cisco dCloud in the following content:
