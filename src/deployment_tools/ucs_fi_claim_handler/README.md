<h1 align="center">UCS FI Claim Handler</h1>
...
<br>
<p align="center">
  The UCS FI Claim Handler for Cisco Intersight automates the claiming of UCS Fabric Interconnect clusters in Intersight Managed Mode (IMM) to an Intersight account.
</p>
<br>

## Features
- Automatically claim UCS Fabric Interconnect clusters in Intersight Managed Mode (IMM) to an Intersight account. Anything that can be done through the Intersight GUI can be done here and more!

- Provide a list of one or more UCS Fabric Interconnect clusters to automatically claim.

- Quickly re-claim and restore UCS Fabric Interconnect cluster targets as part of your on-prem Intersight backup solution.
...
2. Within the unzipped Cisco IMM Automation Tools repository, navigate to the folder \src\deployment_tools\ucs_fi_claim_handler.
3. Edit the ucs_fi_claim_handler.py file to set the **`key_id`** variable using the following instructions:

    **(a).** Open the ucs_fi_claim_handler.py file in an IDLE or text editor of choice.
    ...
    **(d).** Fill in between the quotes of the **`key_id`** variable value with the ID of your API key. For example:
      ...
      ```
4. Edit the ucs_fi_claim_handler.py file to set the **`key`** variable using the following instructions:
    ...
    **(d).** Fill in between the quotes of the **`key`** variable value with your system's file path to the SecretKey.txt file for your API key. For example:
      ...
      ```
5. Edit the ucs_fi_claim_handler.py file to set all the configuration variable values using the following instructions:
...
6. Save the changes you have made to the ucs_fi_claim_handler.py file.
7. Run the ucs_fi_claim_handler.py file.

## Where to Demo
The UCS FI Claim Handler is utilized on Cisco dCloud in the following content:

1. [Cisco Intersight with UCS X-Series Lab](https://dcloud2.cisco.com/demo/cisco-intersight-management-with-x-series-lab-v1)
![Cisco UCS X-Series Lab Topology](../../../src/assets/Cisco_UCS_X-Series_Lab_Topology.png "Cisco UCS X-Series Lab Topology")

Additional related demo content includes:

2. [Cisco Intersight with UCS X-Series Demo](https://dcloud2-rtp.cisco.com/content/instantdemo/cisco-intersight-mode-with-ucs-x-series-v1-instant-demo-2)
