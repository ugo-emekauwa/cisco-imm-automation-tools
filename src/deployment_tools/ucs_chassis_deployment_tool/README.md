<h1 align="center">UCS Chassis Deployment Tool</h1>
...
<br>
<p align="center">
  The UCS Chassis Deployment Tool for Cisco Intersight automates the comprehensive build and deployment of UCS Chassis Profiles, Policies, and Pools for Cisco IMM (Intersight Managed Mode).
</p>
<br>

## Features
- Automatically build one or more UCS Chassis Profiles with all 4 UCS Chassis Policies and requested IP Pools configured to your exact specification. Anything that can be done through the Intersight GUI can be done here and more!

- Automatically assign and deploy one or more UCS Chassis Profiles to UCS Chassis registered under Intersight, using either the serial, name, model, or PID (product ID).

- Quickly restore UCS Chassis Profile, Policy, and Pool configurations as part of your on-prem Intersight backup solution.
...
2. Within the unzipped Cisco IMM Automation Tools repository, navigate to the folder \src\deployment_tools\ucs_chassis_deployment_tool.
3. Edit the ucs_chassis_deployment_tool.py file to set the **`key_id`** variable using the following instructions:

    **(a).** Open the ucs_chassis_deployment_tool.py file in an IDLE or text editor of choice.
    ...
    **(d).** Fill in between the quotes of the **`key_id`** variable value with the ID of your API key. For example:
      ...
      ```
4. Edit the ucs_chassis_deployment_tool.py file to set the **`key`** variable using the following instructions:
    ...
    **(d).** Fill in between the quotes of the **`key`** variable value with your system's file path to the SecretKey.txt file for your API key. For example:
      ...
      ```
5. Edit the ucs_chassis_deployment_tool.py file to set all the configuration variable values using the following instructions:
...
6. Save the changes you have made to the ucs_chassis_deployment_tool.py file.
7. Run the ucs_chassis_deployment_tool.py file.
