<h1 align="center">IMC Access Policy Maker</h1>
...
<br>
<p align="center">
  The IMC Access Policy Maker for Cisco Intersight automates the creation of IMC Access Policies.
</p>
<br>
...
- Automatically attach the IMC Access Policy to one or more UCS Server and/or UCS Chassis Profiles if desired, during the IMC Access Policy configuration.
...
2. Within the unzipped Cisco IMM Automation Tools repository, navigate to the folder \src\policy_makers\imc_access_policy_maker.
3. Edit the imc_access_policy_maker.py file to set the **`key_id`** variable using the following instructions:

    **(a).** Open the imc_access_policy_maker.py file in an IDLE or text editor of choice.
    ...
    **(d).** Fill in between the quotes of the **`key_id`** variable value with the ID of your API key. For example:
      ...
      ```
4. Edit the imc_access_policy_maker.py file to set the **`key`** variable using the following instructions:
    ...
    **(d).** Fill in between the quotes of the **`key`** variable value with your system's file path to the SecretKey.txt file for your API key. For example:
      ...
      ```
5. Edit the imc_access_policy_maker.py file to set all the configuration variable values using the following instructions:
...
6. Save the changes you have made to the imc_access_policy_maker.py file.
7. Run the imc_access_policy_maker.py file.

## Where to Demo
The IMC Access Policy Maker can be demoed on Cisco dCloud in the following content:
