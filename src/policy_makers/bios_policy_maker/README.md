<h1 align="center">BIOS Policy Maker</h1>
...
<br>
<p align="center">
  The BIOS Policy Maker for Cisco Intersight automates the creation of BIOS Policies.
</p>
<br>

## Features
- Automatically build an Intersight BIOS Policy to your exact specification. Anything that can be done through the Intersight GUI can be done here and more!

- Automatically attach the BIOS Policy to a UCS Server Profile if desired, during the BIOS Policy configuration.

- Quickly restore BIOS Policy configurations as part of your on-prem Intersight backup solution.
...
2. Within the unzipped Cisco IMM Automation Tools repository, navigate to the folder \src\policy_makers\bios_policy_maker.
3. Edit the bios_policy_maker.py file to set the **`key_id`** variable using the following instructions:

    **(a).** Open the bios_policy_maker.py file in an IDLE or text editor of choice.
    ...
    **(d).** Fill in between the quotes of the **`key_id`** variable value with the ID of your API key. For example:
      ...
      ```
4. Edit the bios_policy_maker.py file to set the **`key`** variable using the following instructions:
    ...
    **(d).** Fill in between the quotes of the **`key`** variable value with your system's file path to the SecretKey.txt file for your API key. For example:
      ...
      ```
5. Edit the bios_policy_maker.py file to set all the configuration variable values using the following instructions:
...
6. Save the changes you have made to the bios_policy_maker.py file.
7. Run the bios_policy_maker.py file.

## Where to Demo
The BIOS Policy Maker can be demoed on Cisco dCloud in the following content:
