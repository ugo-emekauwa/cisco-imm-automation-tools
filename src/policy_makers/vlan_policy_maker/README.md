<h1 align="center">VLAN Policy Maker</h1>
...
<p align="center">
  The VLAN Policy Maker for Cisco Intersight automates the creation of VLAN Policies.
</p>
<br>

## Features
- Automatically build an Intersight VLAN Policy to your exact specification. Anything that can be done through the Intersight GUI can be done here and more!

- Automatically attach the VLAN Policy to a UCS Domain Profile if desired, during the VLAN Policy configuration. The VLAN Policy can be attached to Fabric Interconnect A, B, or both.
...
2. Within the Cisco IMM Automation Tools repository, navigate to the folder \src\policy_makers\vlan_policy_maker.
3. Edit the vlan_policy_maker.py file to set the **`key_id`** variable using the following instructions:

    **(a).** Open the vlan_policy_maker.py file in an IDLE or text editor of choice.
    ...
    **(d).** Fill in between the quotes of the **`key_id`** variable value with the ID of your API key. For example:
      ...
      ```
3. Edit the vlan_policy_maker.py file to set the **`key`** variable using the following instructions:
    ...
    **(d).** Fill in between the quotes of the **`key`** variable value with your system's file path to the SecretKey.txt file for your API key. For example:
      ...
      ```
4. Edit the vlan_policy_maker.py file to set all the configuration variable values using the following instructions:
...
5. Save the changes you have made to the vlan_policy_maker.py file.
6. Run the vlan_policy_maker.py file.

## Where to Demo
The VLAN Policy Maker can be demoed on Cisco dCloud in the following content:
...
1. [**_Getting Started with Cisco Intersight v2_**](https://dcloud2-rtp.cisco.com/content/instantdemo/getting-started-with-cisco-intersight-v2)
    - **`NOTE:`** Before running the VLAN Policy Maker, change the Policy organization to "Sandpit".
