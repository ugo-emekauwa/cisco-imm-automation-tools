<h1 align="center">IPMI Over LAN Policy Maker</h1>
...
<br>
<p align="center">
  The IPMI Over LAN Policy Maker for Cisco Intersight automates the creation of IPMI Over LAN Policies.
</p>
<br>

## Features
- Automatically build an Intersight IPMI Over LAN Policy to your exact specification. Anything that can be done through the Intersight GUI can be done here and more!

- Automatically attach the IPMI Over LAN Policy to a UCS Server Profile if desired, during the IPMI Over LAN Policy configuration.

- Quickly restore IPMI Over LAN Policy configurations as part of your on-prem Intersight backup solution.
...
2. Within the unzipped Cisco IMM Automation Tools repository, navigate to the folder \src\policy_makers\ipmi_over_lan_policy_maker.
3. Edit the ipmi_over_lan_policy_maker.py file to set the **`key_id`** variable using the following instructions:

    **(a).** Open the ipmi_over_lan_policy_maker.py file in an IDLE or text editor of choice.
    ...
    **(d).** Fill in between the quotes of the **`key_id`** variable value with the ID of your API key. For example:
      ...
      ```
4. Edit the ipmi_over_lan_policy_maker.py file to set the **`key`** variable using the following instructions:
    ...
    **(d).** Fill in between the quotes of the **`key`** variable value with your system's file path to the SecretKey.txt file for your API key. For example:
      ...
      ```
5. Edit the ipmi_over_lan_policy_maker.py file to set all the configuration variable values using the following instructions:
...
6. Save the changes you have made to the ipmi_over_lan_policy_maker.py file.
7. Run the ipmi_over_lan_policy_maker.py file.

## Where to Demo
The IPMI Over LAN Policy Maker can be demoed on Cisco dCloud in the following content:
