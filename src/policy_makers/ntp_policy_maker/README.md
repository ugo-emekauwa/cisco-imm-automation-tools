<h1 align="center">NTP Policy Maker</h1>
...
<p align="center">
  The NTP Policy Maker for Cisco Intersight automates the creation of NTP Policies.
</p>
<br>

## Features
- Automatically build an Intersight NTP Policy to your exact specification. Anything that can be done through the Intersight GUI can be done here and more!

- Automatically attach the NTP Policy to a UCS Domain Profile if desired, during the NTP Policy configuration.

- Quickly restore NTP Policy configurations as part of your on-prem Intersight backup solution.
...
2. Within the Cisco IMM Automation Tools repository, navigate to the folder \src\policy_makers\ntp_policy_maker.
3. Edit the ntp_policy_maker.py file to set the **`key_id`** variable using the following instructions:

    **(a).** Open the ntp_policy_maker.py file in an IDLE or text editor of choice.
    ...
    **(d).** Fill in between the quotes of the **`key_id`** variable value with the ID of your API key. For example:
      ...
      ```
3. Edit the ntp_policy_maker.py file to set the **`key`** variable using the following instructions:
    ...
    **(d).** Fill in between the quotes of the **`key`** variable value with your system's file path to the SecretKey.txt file for your API key. For example:
      ...
      ```
4. Edit the ntp_policy_maker.py file to set all the configuration variable values using the following instructions:
...
5. Save the changes you have made to the ntp_policy_maker.py file.
6. Run the ntp_policy_maker.py file.

## Where to Demo
The NTP Policy Maker can be demoed on Cisco dCloud in the following content:
...
1. [**_Getting Started with Cisco Intersight v2_**](https://dcloud2-rtp.cisco.com/content/instantdemo/getting-started-with-cisco-intersight-v2)
    - **`NOTE:`** Before running the NTP Policy Maker, change the Policy organization to "Sandpit".
