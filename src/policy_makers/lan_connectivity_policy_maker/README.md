<h1 align="center">LAN Connectivity Policy Maker</h1>
...
<br>
<p align="center">
  The LAN Connectivity Policy Maker for Cisco Intersight automates the creation of LAN Connectivity Policies.
</p>
<br>

## Features
- Automatically build an Intersight LAN Connectivity Policy to your exact specification. Anything that can be done through the Intersight GUI can be done here and more!

- Bulk create multiple vNICs within the LAN Connectivity Policy.

- Automatically attach the LAN Connectivity Policy to a UCS Server Profile if desired, during the LAN Connectivity Policy configuration.

- Quickly restore LAN Connectivity Policy configurations as part of your on-prem Intersight backup solution.
...
2. Within the unzipped Cisco IMM Automation Tools repository, navigate to the folder \src\policy_makers\lan_connectivity_policy_maker.
3. Edit the lan_connectivity_policy_maker.py file to set the **`key_id`** variable using the following instructions:

    **(a).** Open the lan_connectivity_policy_maker.py file in an IDLE or text editor of choice.
    ...
    **(d).** Fill in between the quotes of the **`key_id`** variable value with the ID of your API key. For example:
      ...
      ```
4. Edit the lan_connectivity_policy_maker.py file to set the **`key`** variable using the following instructions:
    ...
    **(d).** Fill in between the quotes of the **`key`** variable value with your system's file path to the SecretKey.txt file for your API key. For example:
      ...
      ```
5. Edit the lan_connectivity_policy_maker.py file to set all the configuration variable values using the following instructions:
...
6. Save the changes you have made to the lan_connectivity_policy_maker.py file.
7. Run the lan_connectivity_policy_maker.py file.

## Where to Demo
The LAN Connectivity Policy Maker can be demoed on Cisco dCloud in the following content:
