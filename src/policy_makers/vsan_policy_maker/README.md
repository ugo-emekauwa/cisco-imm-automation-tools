...
2. Within the Cisco IMM Automation Tools repository, navigate to the folder \src\policy_makers\vsan_policy_maker.
3. Edit the vsan_policy_maker.py file to set the **`key_id`** variable using the following instructions:
    ...
    **(d).** Fill in between the quotes of the **`key_id`** variable value with the ID of your API key. For example:
      ...
      ```
3. Edit the vsan_policy_maker.py file to set the **`key`** variable using the following instructions:
    ...
    **(d).** Fill in between the quotes of the **`key`** variable value with your system's file path to the SecretKey.txt file for your API key. For example:
      ...
      ```
4. Edit the vsan_policy_maker.py file to set all the configuration variable values using the following instructions:
...
5. Save the changes you have made to the vsan_policy_maker.py file.
6. Run the vsan_policy_maker.py file.
...
1. [**_Getting Started with Cisco Intersight v2_**](https://dcloud2-rtp.cisco.com/content/instantdemo/getting-started-with-cisco-intersight-v2)
    - **`NOTE:`** Before running the VSAN Policy Maker, change the Policy organization to "Sandpit".
