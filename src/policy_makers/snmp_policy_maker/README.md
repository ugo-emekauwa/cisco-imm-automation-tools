<h1 align="center">SNMP Policy Maker</h1>
...
<br>
<p align="center">
  The SNMP Policy Maker for Cisco Intersight automates the creation of SNMP Policies.
</p>
<br>

## Features
- Automatically build an Intersight SNMP Policy to your exact specification. Anything that can be done through the Intersight GUI can be done here and more!
...
- Automatically attach the SNMP Policy to a UCS Server and/or UCS Domain Profile if desired, during the SNMP Policy configuration.
...
2. Within the unzipped Cisco IMM Automation Tools repository, navigate to the folder \src\policy_makers\snmp_policy_maker.
3. Edit the snmp_policy_maker.py file to set the **`key_id`** variable using the following instructions:

    **(a).** Open the snmp_policy_maker.py file in an IDLE or text editor of choice.
    ...
    **(d).** Fill in between the quotes of the **`key_id`** variable value with the ID of your API key. For example:
      ...
      ```
4. Edit the snmp_policy_maker.py file to set the **`key`** variable using the following instructions:
    ...
    **(d).** Fill in between the quotes of the **`key`** variable value with your system's file path to the SecretKey.txt file for your API key. For example:
      ...
      ```
5. Edit the snmp_policy_maker.py file to set all the configuration variable values using the following instructions:
...
6. Save the changes you have made to the snmp_policy_maker.py file.
7. Run the snmp_policy_maker.py file.

## Where to Demo
The SNMP Policy Maker can be demoed on Cisco dCloud in the following content:
