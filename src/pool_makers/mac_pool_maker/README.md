<h1 align="center">MAC Pool Maker</h1>
...
<p align="center">
  The MAC Pool Maker for Cisco Intersight automates the creation of MAC Pools.
</p>
<br>

## Features
- Automatically build an Intersight MAC Pool to your exact specification. Anything that can be done through the Intersight GUI can be done here and more!

- Set the assignment order of the Pool to sequential order, if desired.

- Quickly restore MAC Pool configurations as part of your on-prem Intersight backup solution.
...
2. Within the Cisco IMM Automation Tools repository, navigate to the folder \src\pool_makers\mac_pool_maker.
3. Edit the mac_pool_maker.py file to set the **`key_id`** variable using the following instructions:

    **(a).** Open the mac_pool_maker.py file in an IDLE or text editor of choice.
    ...
    **(d).** Fill in between the quotes of the **`key_id`** variable value with the ID of your API key. For example:
      ...
      ```
3. Edit the mac_pool_maker.py file to set the **`key`** variable using the following instructions:
    ...
    **(d).** Fill in between the quotes of the **`key`** variable value with your system's file path to the SecretKey.txt file for your API key. For example:
      ...
      ```
4. Edit the mac_pool_maker.py file to set all the configuration variable values using the following instructions:
...
5. Save the changes you have made to the mac_pool_maker.py file.
6. Run the mac_pool_maker.py file.

## Where to Demo
The MAC Pool Maker can be demoed on Cisco dCloud in the following content:
...
1. [**_Getting Started with Cisco Intersight v2_**](https://dcloud2-rtp.cisco.com/content/instantdemo/getting-started-with-cisco-intersight-v2)
    - **`NOTE:`** Before running the MAC Pool Maker, change the Pool organization to "Sandpit".
