<h1 align="center">Resource Pool Maker</h1>

<br>
<p align="center">
  <img alt="Pool Makers Title Image" title="Pool Makers" src="./assets/Pool_Makers_Title_Graphic.png">
</p>  
<br>
<p align="center">
  The Resource Pool Maker for Cisco Intersight automates the creation of Resource Pools.
</p>
<br>

## Features
- Automatically build an Intersight Resource Pool to your exact specification. Anything that can be done through the Intersight GUI can be done here and more!

- Bulk add blade and rack servers quickly to Resource Pools using lists of names and/or serials. Blade servers can also be added by entire domains, chassis IDs and/or chassis slot IDs!

- Set the assignment order of the Pool to sequential order, if desired.

- Quickly restore Resource Pool configurations as part of your on-prem Intersight backup solution.
...
2. Within the unzipped Cisco IMM Automation Tools repository, navigate to the folder \src\pool_makers\resource_pool_maker.
3. Edit the resource_pool_maker.py file to set the **`key_id`** variable using the following instructions:

    **(a).** Open the resource_pool_maker.py file in an IDLE or text editor of choice.
    ...
    **(d).** Fill in between the quotes of the **`key_id`** variable value with the ID of your API key. For example:
      ...
      ```
4. Edit the resource_pool_maker.py file to set the **`key`** variable using the following instructions:
    ...
    **(d).** Fill in between the quotes of the **`key`** variable value with your system's file path to the SecretKey.txt file for your API key. For example:
      ...
      ```
5. Edit the resource_pool_maker.py file to set all the configuration variable values using the following instructions:
...
6. Save the changes you have made to the resource_pool_maker.py file.
7. Run the resource_pool_maker.py file.

## Where to Demo
The Resource Pool Maker can be demoed on Cisco dCloud in the following content:
