<h1 align="center">UUID Pool Maker</h1>
...
<br>
<p align="center">
  The UUID Pool Maker for Cisco Intersight automates the creation of UUID Pools.
</p>
<br>

## Features
- Automatically build an Intersight UUID Pool to your exact specification. Anything that can be done through the Intersight GUI can be done here and more!

- Set the assignment order of the Pool to sequential order, if desired.

- Quickly restore UUID Pool configurations as part of your on-prem Intersight backup solution.
...
2. Within the unzipped Cisco IMM Automation Tools repository, navigate to the folder \src\pool_makers\uuid_pool_maker.
3. Edit the uuid_pool_maker.py file to set the **`key_id`** variable using the following instructions:

    **(a).** Open the uuid_pool_maker.py file in an IDLE or text editor of choice.
    ...
    **(d).** Fill in between the quotes of the **`key_id`** variable value with the ID of your API key. For example:
      ...
      ```
4. Edit the uuid_pool_maker.py file to set the **`key`** variable using the following instructions:
    ...
    **(d).** Fill in between the quotes of the **`key`** variable value with your system's file path to the SecretKey.txt file for your API key. For example:
      ...
      ```
5. Edit the uuid_pool_maker.py file to set all the configuration variable values using the following instructions:
...
6. Save the changes you have made to the uuid_pool_maker.py file.
7. Run the uuid_pool_maker.py file.

## Where to Demo
The UUID Pool Maker can be demoed on Cisco dCloud in the following content:
